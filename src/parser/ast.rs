...
impl ToString for PrefixOperator {
	...
	use PrefixOperator::*;
	match self {
	    Negate => { return "-".to_string(); },
	    Not => { return "!".to_string(); },
	}
    }
}
...
impl ToString for ExpressionType {
	...
	match self {
	    ...
	    Identifier(name) => { return name.clone(); },
	    Int(value) => { return format!("{}", value); },
...
mod tests {
	...
	program.statements.push(StatementType::Let{name: "foo".to_string(), expression: ExpressionType::Int(5)});
